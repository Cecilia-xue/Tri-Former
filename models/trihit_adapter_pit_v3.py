...
class trihit_pit_vitf(trihit_x):
    def __init__(self,
                 in_chans=1,
                 num_classes=100,
                 bk_depths=[2, 2, 4, 2],
    ...
    def forward(self, x):
        ...
        bk_x = bk_x.mean([1,2,3])
        ladder_x = ladder_x.mean([1,2,3])
...
def trihit_cth_pit_v3(num_classes=100, dp_rate=0.15):
    model = trihit_pit_vitf(num_classes=num_classes,
                            bk_depths=[2, 2, 4, 2], bk_dims=[32, 64, 128, 256], bk_arch=['conv', 'hit', 'hit', 'hit'],
                            dp_rate=dp_rate,
