...
class trihit_pit_convf(trihit_x):
    def __init__(self,
                 in_chans=1,
                 num_classes=100,
                 bk_depths=[2, 2, 4, 2],
        ...
        self.adapter_alphas = nn.Parameter(torch.tensor([0.1, 0.9]), requires_grad=True)
    ...
    def forward(self, x):
        ...
        bk_x = bk_x.mean([2,3,4])
        ladder_x = ladder_x.mean([2, 3, 4])
        adapter_alphas = self.adapter_alphas/self.adapter_alphas.sum()
        x = adapter_alphas[0] * bk_x + adapter_alphas[1] * ladder_x
...
def trihit_cth_pit_v2(num_classes=100, dp_rate=0.15):
    model = trihit_pit_convf(num_classes=num_classes,
                             bk_depths=[2, 2, 4, 2], bk_dims=[32, 64, 128, 256], bk_arch=['conv', 'hit', 'hit', 'hit'],
                             dp_rate=dp_rate,
